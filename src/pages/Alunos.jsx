    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Gerir os alunos matriculados</p>
        </div>
        <button
          onClick={openCreate}
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-16 text-center shadow-sm">
          <CircleUser size={40} className="mx-auto text-gray-300 dark:text-slate-600 mb-3" />
          <p className="text-gray-500 dark:text-slate-400 font-medium">Nenhum aluno encontrado</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/40">
                ...
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide hidden sm:table-cell">Turma</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide hidden md:table-cell">Data de Nascimento</th>
                <th className="px-6 py-3" />
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
              {filtered.map((aluno) => (
                <tr key={aluno.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition text-gray-700 dark:text-slate-300">
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 bg-blue-100 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0">
                        {aluno.nome_completo.split(' ').map(n => n[0]).slice(0, 2).join('')}
                      </div>
                      <span className="font-medium text-gray-800 dark:text-slate-100">{aluno.nome_completo}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-gray-500 dark:text-slate-400 hidden sm:table-cell">
                    {aluno.turmas
                      ? `${aluno.turmas.classes?.nome || ''} - ${aluno.turmas.nome}`
                      : <span className="text-gray-300 dark:text-slate-600 italic">Sem turma</span>}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-end space-x-1">
                      <button onClick={() => openEdit(aluno)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 transition cursor-pointer text-gray-500 dark:text-slate-400">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => handleDelete(aluno.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 transition cursor-pointer text-red-500 dark:text-red-400">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
      ...
      {showModal && (
        <Modal title={editing ? 'Editar Aluno' : 'Novo Aluno'} onClose={() => setShowModal(false)}>
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Data de Nascimento</label>
              <input
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Turma</label>
              <select
                value={form.turma_id}
